...
func main() {
	k, a, b := nextInt(), next(), next()
	aarr := strings.Split(a, "")
	barr := strings.Split(b, "")
	aans := convertNumSystem(aarr, k)
	bans := convertNumSystem(barr, k)
	fmt.Println(aans * bans)
}

func convertNumSystem(arr []string, k int) int {
	var ans int
	for i := 0; i < len(arr); i++ {
		n, _ := strconv.Atoi(arr[i])
		ans += n * pow(k, len(arr)-i-1)
	}
	return ans
}
