...
func main() {
	...
	var ans string
	if len(arr) == 1 {
		a1, _ := strconv.Atoi(arr[0])
		ans = hundredItoE(a1)
	} else if len(arr) == 2 {
		a2, _ := strconv.Atoi(arr[1]) + "thousand"
		a1, _ := strconv.Atoi(arr[0])
		ans = a2 + a1
	} else if len(arr) == 3 {
		a2, _ := strconv.Atoi(arr[1]) + "thousand"
